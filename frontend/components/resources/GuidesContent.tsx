/**
 * GuidesContent — rendered inside /resources/guides.
 * Full Knowledge Centre: guides, documentation preview, FAQs preview, downloads, latest blog.
 */
...
const faqSnippets = [
  { q: 'Who qualifies for the ₹1.38 Lakh combined subsidy?',         href: '/resources/faqs' },
  { q: 'Is BESS mandatory for my rooftop solar system?',             href: '/resources/faqs' },
  { q: 'How long does the full installation process take in Odisha?', href: '/resources/faqs' },
  { q: 'Can I use imported panels and still claim subsidies?',        href: '/resources/faqs' },
  { q: 'What is net metering and how are year-end credits paid?',     href: '/resources/faqs' },
];
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-12">
        {[
          ...
          { label: 'FAQs',          href: '/resources/faqs',     icon: '❓' },
          { label: 'Downloads',     href: '#downloads', icon: '📥' },
          { label: 'Blog',          href: '/resources/blogs',    icon: '✍️' },
        ].map((item) => (
          <Link
            key={item.label}
            href={item.href}
      ...
      {/* ── Guides ─────────────────────────────────────────────────────── */}
      <section id="guides" className="mb-16" aria-labelledby="guides-heading">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/resources/blogs" className="text-sm font-semibold text-[var(--accent)] hover:underline">View all articles →</Link>
        </div>
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 list-none p-0 m-0" role="list">
          {guides.map((guide) => (
            <li key={guide.title}>
              <Link
                href={guide.href}
                className="group block rounded-xl border border-[var(--border)] bg-[var(--bg-subtle)] p-6 h-full flex flex-col gap-3 hover:border-[var(--accent)]/40 hover:shadow-[var(--shadow-md)] transition-all duration-200"
              >
      ...
      {/* ── Documentation ──────────────────────────────────────────────── */}
      <section id="docs" className="mb-16 rounded-2xl bg-[var(--bg-subtle)] border border-[var(--border)] p-8" aria-labelledby="docs-heading">
        <div className="mb-8">
          ...
          {docs.map((doc) => (
            <li key={doc.title} className="rounded-xl border border-[var(--border)] bg-[var(--bg)] p-6 flex gap-4 items-start">
              ...
              <div className="flex-1 min-w-0">
                ...
                <Link href="/businesses/rooftop-solar" className="inline-block mt-2 text-xs font-semibold text-[var(--accent)] hover:underline">View details →</Link>
              </div>
            </li>
          ))}
        </ul>
        <div className="mt-6 text-center">
          <Link href="/resources/documentation" className="text-sm font-semibold text-[var(--accent)] hover:underline">
      ...
      {/* ── FAQs preview ───────────────────────────────────────────────── */}
      <section className="mb-16" aria-labelledby="faq-preview-heading">
        <div className="flex items-center justify-between mb-8">
          <div>
          ...
          <Link href="/resources/faqs" className="text-sm font-semibold text-[var(--accent)] hover:underline">View all FAQs →</Link>
        </div>
        <ul className="space-y-2 list-none p-0 m-0" role="list">
          {faqSnippets.map((faq) => (
            <li key={faq.q}>
              <Link
                href={faq.href}
                className="flex items-center justify-between gap-4 rounded-xl border border-[var(--border)] bg-[var(--bg-subtle)] px-5 py-4 hover:border-[var(--accent)]/40 hover:bg-[var(--bg)] transition-all duration-150 group"
              >
      ...
      {/* ── Downloads ──────────────────────────────────────────────────── */}
      <section id="downloads" className="mb-16 rounded-2xl bg-[var(--bg-subtle)] border border-[var(--border)] p-8" aria-labelledby="downloads-heading">
        <div className="mb-8">
      ...
      <section aria-labelledby="latest-heading">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/resources/blogs" className="text-sm font-semibold text-[var(--accent)] hover:underline">View all →</Link>
        </div>
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 list-none p-0 m-0" role="list">
          {solarBlogPosts.map((post) => (
            <li key={post.slug}>
              <Link
                ...
                className="group block rounded-xl border border-[var(--border)] bg-[var(--bg-subtle)] p-5 h-full flex flex-col gap-2 hover:shadow-[var(--shadow-md)] hover:-translate-y-0.5 transition-all duration-200"
              >
                <div className="flex flex-wrap gap-1.5">
                  {post.tags.slice(0, 2).map((tag) => (
                    <span key={tag} className="text-[10px] font-semibold text-[var(--accent)] uppercase tracking-wider">{tag}</span>
                  ))}
                </div>
      ...
      <div className="mt-16 rounded-2xl border border-[var(--border)] bg-[var(--bg-subtle)] py-12 text-center max-w-xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link href="/businesses/rooftop-solar/solar-calculator" className="inline-flex items-center justify-center h-11 px-6 rounded-lg text-sm font-semibold bg-[var(--btn-primary-bg)] text-[var(--btn-primary-fg)] hover:bg-[var(--btn-primary-hover)] transition-colors">
            Try Solar Calculator
          </Link>
          <Link href="/consultation" className="inline-flex items-center justify-center h-11 px-6 rounded-lg text-sm font-semibold border border-[var(--border)] text-[var(--fg)] hover:bg-[var(--bg-muted)] transition-colors">
